...
export default async function DetailedStructure() {
  const elements = await client.fetch<SanityDocument[]>(
    POSTS_QUERY,
    {},
    options
  );
    ...
    <div className="container mx-auto p-4 space-y-6 text-gbBlack">
      {Object.keys(groupedElements).map((type) => (
        <div key={type} className="space-y-4">
          ...
          {type === "networking" && <ColorfulTitle text="NETWORKING" />}
          {type === "talleres" && <ColorfulTitle text="TALLERES" />}
          {type === "mentores" && <ColorfulTitle text="MENTORES" />}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {groupedElements[type].map((element) => (
              <div key={element._id} className="border rounded-lg">
                <ProgramasComponent
                  photo={
                    element.image && element.image._upload
                      ? element.image._upload.previewImage
                      : null
                  }
                  emoji={element.emoji}
                  title={element.title}
