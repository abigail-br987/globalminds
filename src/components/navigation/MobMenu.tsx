...
export default function MobMenu({ menuItems }: MobMenuProps) {
  ...
  return (
    <div>
      ...
      >
        <ul>
          {menuItems.map(({ name, subMenu }, i) => {
            const isClicked = clicked === i;
            const hasSubMenu = subMenu?.length;
            return (
              <li key={name} className="">
                <span
                  className="flex items-center justify-between-between p-4 hover:bg-white/5 rounded-md cursor-pointer relative"
