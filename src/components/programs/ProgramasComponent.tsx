...
const ProgramasComponent: React.FC<Props> = ({
  ...
  return (
    <div
      className={`rounded-lg h-full bg-gbWhite relative ${past ? "opacity-70 cursor-not-allowed" : ""}`}
    >
      {past && (
        <h3
          className={`absolute top-0 font-gotaRegular left-0 w-full py-3 px-1 bg-${color} text-center z-10`}
        >
          FINALIZADO
        </h3>
      )}
